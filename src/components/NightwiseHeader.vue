...
const authenticateWithProvider = async (provider: string) => {
  try {
    const authData = await base.authenticateWithProvider(provider);
    ...
    <Dialog v-if="!authStatus">
      ...
      <DialogContent class="max-w-xs rounded-md sm:max-w-[375px]">
        ...
        <div class="flex justify-evenly gap-4">
          <div
            @click="() => authenticateWithProvider('github')"
          ...
          >
            ...
            <p class="font-semibold text-primary">Login with GitHub</p>
          </div>
          <div
            @click="() => authenticateWithProvider('discord')"
            class="flex w-full cursor-pointer items-center justify-between gap-4 rounded-md bg-secondary px-4 py-3 text-xs duration-300 ease-in-out hover:scale-105 sm:text-base"
          >
            <svg
              fill="#ffffff"
            ...
            <p class="font-semibold text-primary">Login with Discord</p>
          </div>
        </div>
      </DialogContent>
    </Dialog>
